...
for each_topic in TOPIC_DICT:
    ...
    for eachele in TOPIC_DICT[each_topic]:
        try:
            ...
            print( FUNC_TEMPLATE.replace("CURRENTTOPIC",CURRENTTOPIC.replace(".","").replace(":","")).replace("CURRENTINDEX",CURRENTINDEX).replace("CURRENTTITLE",CURRENTTITLE).replace("CURRENTHTML",CURRENTHTML).replace("NEXTINDEX",NEXTINDEX) )
